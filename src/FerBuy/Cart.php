...
class Cart {
	...
	private function setCurrency($currency) {
		if (strlen($currency) == 3){
			...
			throw new Tp\InvalidArgumentException('Currency must be exactly 3 characters long');
		}
	}
	...
	public function setTax($tax) {
		if (!is_numeric($tax)) {
			throw new Tp\InvalidArgumentException('Tax has to be numeric.');
		}
	...
	public function setDiscount($discount) {
		if (!is_numeric($discount)) {
			throw new Tp\InvalidArgumentException('Discount has to be numeric.');
		}
	...
	public function setShipping($shipping) {
		if (!is_numeric($shipping)) {
			throw new Tp\InvalidArgumentException('Shipping has to be numeric.');
		}
	...
	private function getNumberWithoutDecimals($number) {
		if(!is_numeric($number)) {
			throw new Tp\InvalidArgumentException("Number has to be numeric");
		}
