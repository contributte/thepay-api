...
class RadioMerchant {
	/**
	 * @var Tp\MerchantConfig merchant configuration
	 */
	protected $config;
	protected $name;
	protected $value;
	 ...
	 */
	function __construct(Tp\MerchantConfig $config, $name=null, $value=null, $showIcon = true, $disablePopupCss = false, $currency = null) {
		$this->config = $config;
		$this->name = $name;
		$this->value = $value;
	 ...
	 * @param Tp\MerchantConfig $config merchant configuration
	 */
	public function setConfig(Tp\MerchantConfig $config) {
		$this->config = $config;
	}
	...
	public function renderRadio(){
		...
		if($this->appendCode) {
			$thepayAppendCode = Tp\Escaper::jsonEncode($this->appendCode);
	 ...
	 *
	 * @param Tp\Payment $payment data of payment
	 ...
	 * @throws Tp\Exception if headers was already sent
	 */
	public function redirectOnlinePayment(Tp\Payment $payment, $redirectFunc = null, $dieAfterRedirect = false, $forcedValue = null) {
		if((!empty($_REQUEST['tp_radio_value']) || $forcedValue > 0) && empty($_REQUEST['tp_radio_is_offline'])){
			if(headers_sent()){
				throw new Tp\Exception('Redirect error - headers have been already sent');
			}
			$this->clearCookies();
	 ...
	 * @deprecated
	 */
	public function redirectOfflinePayment(Tp\Payment $payment, $redirecFunc = null){
	 ...
	 *
	 * @param Tp\Payment $payment
	 * @return string HTML code with component
	 */
	public function showPaymentInstructions(Tp\Payment $payment){
		...
		$thepayDisablePopupCss = Tp\Escaper::jsonEncode($this->disablePopupCss);
